...
Redux は先行する幾つもの技術から受け継いだものがあります。ある種のパターンや技術の点では先行する技術と似ていますが、同時に重要な違いもあります。そういった共通点と違いについて深く検討していきます。
...
Redux は Flux の実装の一方式だといえるのでしょうか？ある点ではそうですし、ある点では違います。
...
Redux はいくつか重要な Flux の性質から影響を受けています。Redux は Flux 同様、アプリケーションの特定のレイヤーのモデルの更新に関する logic を集中管理し、またそれを前もって定義しておきます。\(Flux においては"stores", Redux においては"reducers"です。\) どちらのシステムも、アプリケーションに直接的にデータを変更させる\(訳注：mutate\)のではなく、全ての変更を単なるオブジェクトである"action"によって記述するようになっています。
